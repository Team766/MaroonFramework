	...
	public String handle(final Map<String, Object> params) {
		...
		if (selectedAutonModeName != null) {
			final Optional<AutonomousMode> selectedAutonMode = Arrays.stream(m_autonModes)
					.filter(m -> m.name().equals(selectedAutonModeName)).findFirst();
			if (selectedAutonMode.isEmpty()) {
				Logger.get(Category.AUTONOMOUS).logData(Severity.ERROR,
						...
						selectedAutonModeName, Arrays.stream(m_autonModes).map(m -> m.name())
		...
		return String.join("\n", new String[]{
			...
			"<form>",
			"<p>" + HtmlElements.buildDropDown("AutoMode", selectedAutonModeNameUI, Arrays.stream(m_autonModeNames)::iterator) + "</p>",
